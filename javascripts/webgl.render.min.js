...
this.gl.enableVertexAttribArray(a);this.gl.bindBuffer(this.gl.ARRAY_BUFFER,this.gl.createBuffer());this.gl.vertexAttribPointer(a,3,this.gl.FLOAT,!1,12,0);this.lastDrawTime=performance.now();requestAnimationFrame(this.draw)};d.prototype.draw=function(){var a=performance.now(),b=this.gl;this.updateIterationCount(a-this.lastDrawTime);for(var c=0;c<this.iterations;c++)this.timeStep(this.displayWidth,this.displayHeight);b.bufferData(b.ARRAY_BUFFER,this.circleFloat32Array,b.DYNAMIC_DRAW);b.drawArrays(b.POINTS,
0,this.circleCount);this.lastDrawTime=a;requestAnimationFrame(this.draw)};d.prototype.updateIterationCount=function(a){17>a&&32>this.iterations?(this.iterationConvergence=0,this.iterations*=2):20>a&&64>this.iterations?(this.iterationConvergence=0,this.iterations++):55<a&&1<this.iterations&&(this.iterationConvergence=0,this.iterations--);10>this.iterationConvergence&&(this.iterationConvergence++,10===this.iterationConvergence&&this.convergedIterations!==this.iterations&&(this.convergedIterations=this.iterations,
console.log("timeStep of "+this.iterations+" iterations/frame, ~"+Math.round(1E3/a)+" frames/sec.")))};d.initDevice=function(a,b){a.attachShader(b,c.shaders.compileShader(a,a.VERTEX_SHADER,c.shaders.VERTEX_SHADER));a.attachShader(b,c.shaders.compileShader(a,a.FRAGMENT_SHADER,c.shaders.FRAGMENT_SHADER));a.linkProgram(b);if(!a.getProgramParameter(b,a.LINK_STATUS))throw"Unable to link program:  "+a.getProgramInfoLog(b);a.useProgram(b);a.blendFunc(a.ONE,a.ONE_MINUS_SRC_ALPHA);a.enable(a.BLEND)};d.prototype.resize=
function(){var a=this.gl.canvas;this.displayWidth=a.clientWidth;this.displayHeight=a.clientHeight;if(a.width!==this.displayWidth||a.height!==this.displayHeight)a.width=this.displayWidth,a.height=this.displayHeight,this.gl.viewport(0,0,this.displayWidth,this.displayHeight),this.gl.uniform2f(this.u_resolutionLoc,a.width,a.height)};c.render=d})(webgl||(webgl={}));
